...
def get_week_ranking(ar=False,start_date=now_week[0], end_date=now_week[1]):
    ranking_type = "achievement_rate" if ar else "rank_score"
    return sql_execute("with data_1 as ( select s.user_id AS user_id, u.username AS username, u.country_code AS country_code, count(s.user_id) AS play_count, ( sum(s.accuracy) / ( select count.beatmaps from 840mt.count ) ) AS achievement_rate, avg(s.accuracy) AS Accuracy, sum(s.score) AS total_score from ( 840mt.scores s join 840mt.users u ) where (s.user_id = u.user_id) and ( s.date > '{0}' and s.date < '{1}' ) group by u.user_id ), data_2 as ( select s.user_id AS user_id, s.beatmap_id AS beatmap_id, s.score AS bestscore, s.rank AS 'rank' from ( 840mt.scores s join ( select s.user_id AS user_id, s.beatmap_id AS beatmap_id, max(s.score) AS bestscore from 840mt.scores s group by s.beatmap_id, s.user_id ) temp on( ( (s.user_id = temp.user_id) and (s.beatmap_id = temp.beatmap_id) and (s.score = temp.bestscore) ) ) ) where ( s.date > '{0}' and s.date < '{1}' ) ), data_3 as ( select d.user_id AS user_id, sum(d.bestscore) AS rank_score, sum( ( case when ( (d.rank = 'X') or (d.rank = 'XH') ) then 1 else 0 end ) ) AS SS, sum( ( case when ( (d.rank = 'S') or (d.rank = 'SH') ) then 1 else 0 end ) ) AS S, sum( ( case when (d.rank = 'A') then 1 else 0 end ) ) AS A from ( data_2 d join 840mt.users u ) where (d.user_id = u.user_id) group by u.user_id ), week_ranking AS ( select a.user_id AS user_id, a.username AS username, a.country_code AS country_code, a.play_count AS play_count, a.achievement_rate AS achievement_rate, a.Accuracy AS accuracy, a.total_score AS total_score, b.rank_score AS rank_score, b.SS AS SS, b.S AS S, b.A AS A from ( data_1 AS a left join data_3 b on((a.user_id = b.user_id)) ) ) SELECT rank() OVER ( ORDER BY {2} desc ) AS 'rank', week_ranking.* from week_ranking".format(start_date, end_date, ranking_type))
